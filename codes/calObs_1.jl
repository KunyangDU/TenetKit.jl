...
#         Threads.@spawn while isopen(ch)
#             # @show Base.gc_live_bytes()/2^30
#             # @show Base.summarysize(ch)/2^30
#             # @show Base.summarysize(ch_swap)/2^30
...
function _calObs_threading!(Obs::Observable, obj::Union{DenseMPO,DenseMPS};kwargs...)
    ...
    ch = Channel{Union{AbstractObservableTreeNode,String}}(cachesize)
    ch_swap = Channel{Union{AbstractObservableTreeNode,String}}(Inf)
    ch_info = Channel{Tuple{Int64,TimerOutput}}(Inf)
                ...
                for node in p.children 
                    ...
                    if length(task) < 1
                        ...
                        if isdisk
                            ...
                            serialize(io, node)
                            close(io)
                            @timeit to "put!" put!(ch_swap,filepath)
                            # node.Env = nothing
                        else
