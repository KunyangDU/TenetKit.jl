...
SETTN1!(lsβ[1], H, ρ;
trunc = truncdim(Ds) & truncbelow(1e-6),tol = 1e-6,
algo = CBEalgo(dynamicSVD(1.2,2),NoStruc(),0,Ds,1e-6),max_order = 10)
# SETTN2!(lsβ[1], H, ρ;
# trunc = truncdim(Ds) & truncbelow(1e-6),tol = 1e-6,max_order = 10)
Z = normalize!(ρ)^2
