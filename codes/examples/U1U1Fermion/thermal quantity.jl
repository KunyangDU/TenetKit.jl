...
Latt = YCSqua(Lx,Ly)
L = size(Latt)
@load "examples/U1U1Fermion/data/Latt_$(Lx)x$(Ly).jld2" Latt

D = 2^6
params = (μ=0,U=0)
H= Hamiltonian(Latt;params...)

@load "examples/U1U1Fermion/data/lsβ_$(Lx)x$(Ly)_$(D)_$(params).jld2" lsβ
@load "examples/U1U1Fermion/data/lsρ_$(Lx)x$(Ly)_$(D)_$(params).jld2" lsρ
f = zeros(length(lsβ))
u = zeros(length(lsβ))
u2 = zeros(length(lsβ))
lsβ *= 2
...
save("examples/U1U1Fermion/figures/thermal_quant_$(Lx)x$(Ly)_D=$(D).pdf",fig)
save("examples/U1U1Fermion/figures/thermal_quant_$(Lx)x$(Ly)_D=$(D).png",fig)
