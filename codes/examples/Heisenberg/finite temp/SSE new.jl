...
mutable struct ObservableTreeLeave{N}
    ...
    value::Number
    function ObservableTreeLeave(A::InteractionTreeLeave, i::Int64, N::Int64)
        site = Vector(undef,N)
        name = Vector(undef,N)
        site[i] = A.site
        name[i] = A.name
...
function relevent_node(root::InteractionTreeNode, site::Int64, dismiss::Function = (x -> false))
    ...
    # # for l in filter(x -> site in x.Leave.site, Leaves(root))
...
mutable struct CompositeObservableTreeNode{N} <: AbstractTreeNode where {N}
    A::NTuple{N,Any}
