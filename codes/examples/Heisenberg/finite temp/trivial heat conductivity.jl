...
function contract(EnvL::LeftEnvironmentTensor{2},ht::LocalOperator{1,1},t::DenseMPOTensor{4},hb::IdentityOperator{1},t′::AdjointMPOTensor{4},EnvR::RightEnvironmentTensor{2})
    return @tensor EnvL.A[3,2] * ht.A[1,4] * t.A[5,2,6,1] * t′.A[7,4,5,3] * EnvR.A[6,7]
end

function contract(EnvL::LeftEnvironmentTensor{2},::IdentityOperator{1},t::DenseMPOTensor{4},hb::IdentityOperator{1},t′::AdjointMPOTensor{4},EnvR::RightEnvironmentTensor{2})
...
function pushleft(h::LocalOperator{1, 1}, A::DenseMPOTensor{4}, ::Nothing, A′::AdjointMPOTensor{4}, EnvR::RightEnvironmentTensor{2})
    @tensor tmp[-1;-2] ≔ h.A[1,5] * A.A[6,-1,2,1] * A′.A[3,5,6,-2] * EnvR.A[2,3]
...
function pushleft(::IdentityOperator{1}, objt::DenseMPOTensor{4}, ::IdentityOperator{1}, objb::AdjointMPOTensor{4}, EnvR::RightEnvironmentTensor{2})
    @tensor x[-1;-2] ≔ objt.A[2,-1,3,1] * objb.A[4,1,2,-2] * EnvR.A[3,4]
    return RightEnvironmentTensor(x)
end
...
# Env.envs = Vector{AbstractEnvironmentTensor}(undef,size(Latt) + 1)
# # Array{AbstractEnvironmentTensor}(undef, 1, 1)
initialize!(Env)
_scalar(Env)
# Env.envs[1]
# initialize!(Env)
