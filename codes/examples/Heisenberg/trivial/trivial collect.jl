...
filepath = "examples/Heisenberg/trivial/data/lkan"
dataname = filepath
    ...
    for iψ in 1:3
        Op = let Sop = symbs[iψ]
            ...
            for i in 1:size(Latt)
                addIntr!(Root,TrivialSpinOneHalf.eval(Sop),i,string(Sop),false,exp(1im * dot(k,coordinate(Latt,i))),nothing)
            end
            AutomataSparseMPO(Root,size(Latt))  
        end

        if isfile("$(dataname)/ψ′_$(xyznames[iψ])_$(Lx)x$(Ly)_$(D)_$(params)_$(k/pi)_$(t₀).jld2")
            @load "$(dataname)/ψ′_$(xyznames[iψ])_$(Lx)x$(Ly)_$(D)_$(params)_$(k/pi)_$(0.0).jld2" ψ′
            ψ₀ = deepcopy(ψ′)
            @load "$(dataname)/ψ′_$(xyznames[iψ])_$(Lx)x$(Ly)_$(D)_$(params)_$(k/pi)_$(t₀).jld2" ψ′
        else
            @assert t₀ ≈ 0
            ψ′ = mul!(deepcopy(ψ),ψ,Op ,1,Algebraalgo(DoubleSite(),NoAlgorithm(),truncdim(D) & truncbelow(1e-12),3,1e-8))[1]
            @save "$(dataname)/ψ′_$(xyznames[iψ])_$(Lx)x$(Ly)_$(D)_$(params)_$(k/pi)_$(t₀).jld2" ψ′
            ψ₀ = deepcopy(ψ′)
        end
        ...
        for i in 1:Nt
            ...
            flush(stdout)
            TDVP!(Env, alg, info)
            # @save "$(dataname)/info_$(xyznames[iψ])_$(Lx)x$(Ly)_$(D)_$(params)_$(k/pi)_$(t).jld2" info
            lsSS[i+1] = inner(Env.layer[1],ψ₀') * exp(1im * t * lsEg[end])
        end
        # @save "$(dataname)/ψ′_$(xyznames[iψ])_$(Lx)x$(Ly)_$(D)_$(params)_$(k/pi)_$(t₀ + Nt*τ).jld2" ψ′
        # @save "$(dataname)/lsSS_$(xyznames[iψ])_$(Lx)x$(Ly)_$(D)_$(params)_$(k/pi)_$(t₀)_$(t₀ + Nt*τ).jld2" lsSS
    end
    GC.gc()
end