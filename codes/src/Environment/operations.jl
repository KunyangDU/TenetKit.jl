...
function setdefault!(env::Environment{3};kwargs...)
    ...
    if issparse(env.layer[2])
        ...
        env.envs[1] = SparseLeftEnvironmentTensor(one(S) * isometry(lds...))
        env.envs[end] = SparseRightEnvironmentTensor(one(S) * isometry(rds...))
    else
        AuxSpaces = reverse(map(x -> getAuxSpace(env.layer[x].ts[1])[1],1:3))
        env.envs[1] = DenseLeftEnvironmentTensor(one(S) * isometry(AuxSpaces[1],AuxSpaces[2] ⊗ AuxSpaces[3]))
        AuxSpaces = map(x -> getAuxSpace(env.layer[x].ts[end])[2],1:3)
        env.envs[end] = DenseRightEnvironmentTensor(one(S) * isometry(AuxSpaces[1] ⊗ AuxSpaces[2], AuxSpaces[3]))
    end
end
...
function setdefault!(env::Environment{2};kwargs...)
    ...
    if !issparse(env.layer[1]) && !issparse(env.layer[2])
        env.envs[1] = DenseLeftEnvironmentTensor(one(S) * isometry(map(x -> getAuxSpace(env.layer[x].ts[1])[1],1:2)...))
        env.envs[end] = DenseRightEnvironmentTensor(one(S) * isometry(map(x -> getAuxSpace(env.layer[x].ts[end])[2],1:2)...))
...
function setdefault!(Env::Environment{4})
    ...
    if issparse(Env.layer[1]) && issparse(Env.layer[3])
        ρ1 = Env.layer[2]
        ρ2′ = Env.layer[4]
        tmpL = one(S) * isometry(space(ρ2′.ts[1])[4]', space(ρ1.ts[1])[2])
        tmpR = one(S) * isometry(space(ρ1.ts[end])[3]', space(ρ2′.ts[end])[1])
