...
mutable struct InteractionGraph{L,G<:AbstractGraph}
    ...
    function InteractionGraph(tunnel::Vector{<:InteractionTunnel{L}}) where L
        return new{L, DirectedAcyclicGraph}(nothing, tunnel, nothing, L)
    end

    function InteractionGraph(L::Int64)
        return new{L, DirectedAcyclicGraph}(nothing, Vector{InteractionTunnel{L}}(), nothing, L)
    end
end
...
function build_graph!(ig::InteractionGraph{L}) where L
    ...
    ig.graph = optimize!(DirectedAcyclicGraph(ig.tunnel))
    return ig
end
