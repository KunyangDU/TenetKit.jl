mutable struct ObservableOperator{R₁,R₂} <: AbstractLocalOperator{R₁,R₂}
    ...
    ObservableOperator(A::AbstractLocalOperator{R₁,R₂}, isstring::Bool) where {R₁,R₂} = new{R₁,R₂}(A,isstring,nothing,nothing,nothing,nothing)
    ObservableOperator(A::AbstractTensorMap, name::String, site::Int64, isstring::Bool = false) = new{length(codomain(A)),length(domain(A))}(LocalOperator(A, name, site),isstring,nothing,nothing,nothing,nothing)
end
