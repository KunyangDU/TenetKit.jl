function bDMRG2!(ψ::DenseMPS, H::SparseMPO, D_MPS::Int64;
    kwargs...)
    @time "Initialize Environment" begin
        Env = Environment([ψ,H,adjoint(ψ)])
        initialize!(Env)
    end
    ~,truncerror = DMRG2!(Env, D_MPS;kwargs...,tol = 1e-16,return_error = true)
    println("Benchmark begin with truncerror = $(truncerror)")
    lsE,btrial = bDMRG2!(Env, D_MPS;kwargs...)
    return lsE,btrial
end

function bDMRG2!(Env::Environment{3}, D_MPS::Int64;
