...
function randSVD!(env::Environment{3}, alg::CBEalgo,info::CBEinfo{L2R})
    ...
    Ω = _cbetensor(randn,B,D_f - D_i,:right)
    ...
    if norm(_cbeinner(obj,B,:right)) >= 1
        data = (EnvL,EnvR,A,Λ,B,Ω)
        @save "testtensor.jld2" data
    end
...
function randSVD!(env::Environment{3},alg::CBEalgo,info::CBEinfo{R2L})
    ...
    Ω = _cbetensor(randn,A,D_f - D_i,:left)
    ...
    @timeit localto "contract" obj = contract(Lorth,Rorth)
    if norm(_cbeinner(obj,A,:left)) >= 1
        data = (EnvL,EnvR,A,Λ,B,Ω)
        @save "testtensor.jld2" data
    end
