...
K = 1
lsΓ = [0.0,-0.08,-0.16,-0.2,-0.24,-0.28,-0.32,-0.36,-0.4]

lsHf = 0:0.04:0.8


lsχ = zeros(length(lsHf)-2,length(lsΓ))
...
lsχ[:,j] = [-(lsE[i] + lsE[i+2] - 2lsE[i+1])/(lsHf[i+2] - lsHf[i+1])/(lsHf[i+1] - lsHf[i]) for i in 1:length(lsE)-2]
...
ax = Axis(fig[1,1],
yticks = -lsΓ)
heatmap!(ax,lsHf[1:end-2],-lsΓ,lsχ)
