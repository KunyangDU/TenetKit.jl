...
# lsϕ = 2pi*(0:0.1:0.9)
# lsϕ = 2pi*[-0.1,0.0,0.1]
lsϕ = 2pi*[-0.005,0.0,0.005]

Ftotal = zeros(length(lsHB),length(lsϕ),length(lsβ2eff))
ktotal = zeros(length(lsHB),length(lsϕ)-2,length(lsβ2eff))

for (iHB,HB) in enumerate(lsHB), (iϕ,ϕ) in enumerate(lsϕ)
# HB = lsHB[1]
# ϕ = lsϕ[2]

Hx = round(HB*sin(θ)*cos(ϕ);digits = 3)
Hy = round(HB*sin(θ)*sin(ϕ);digits = 3)
Hz = round(HB*cos(θ);digits = 3)
@show Hx,Hy,Hz
...
ktotal = map(1:length(lsϕ)-2) do i 
    (2Ftotal[:,i+1,:]-Ftotal[:,i,:]-Ftotal[:,i+2,:]) / (lsϕ[i+2] - lsϕ[i])
end

lskdT = ktotal[1] .* (ones(length(lsHB)) .* lsβ2eff') 
lsBdT = lsHB .* lsβ2eff'
...
fig = Figure()
# # axF = Axis(fig[1,1])
figsize = (height = 300,width = 400)

axkdT = Axis(fig[1,1];figsize...,
xlabel = L"B/T",
ylabel = L"k/T",
# xscale = log10
...
# xscale = log10)
for i in eachindex(lsHB)
    scatterlines!(axkdT, lsBdT[i,:],lskdT[i,:],label = "B = $(lsHB[i])")
end
...
Legend(fig[1,2],axkdT)

resize_to_layout!(fig)
display(fig)


save("$(figurename)/kdT_$(Lx)x$(Ly)_$(D)_$(params_Kitaev).png",fig)
save("$(figurename)/kdT_$(Lx)x$(Ly)_$(D)_$(params_Kitaev).pdf",fig)
