...
lsflux[i] = fluxvalues |> x -> sum(x)/length(x)
end
# end
...
scatterlines!(ax,lsHf,lsflux*2^6,label = "Γ′=$(Γ′)")
...
save("$(figurename)/flux_$(Lx)x$(Ly)_$(D)_$(θ/pi)_$(ϕ/pi).png",fig)
save("$(figurename)/flux_$(Lx)x$(Ly)_$(D)_$(θ/pi)_$(ϕ/pi).pdf",fig)
