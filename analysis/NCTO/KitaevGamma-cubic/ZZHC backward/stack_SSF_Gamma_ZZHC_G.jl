...
lines!(ax,collect(extrema(lsHf)),0.5*[1,1] .+ i*0.25,linestyle = :dash)
scatterlines!(ax,lsHf,lsSSFp[i,:] .+ i*0.25,label = "Γ=$(Γ)")
...
save("$(figurename)/stack_SSF_$(Lx)x$(Ly)_$(D)_$(θ/pi)_$(ϕ/pi)_$(K)_Gamma.png",fig)
save("$(figurename)/stack_SSF_$(Lx)x$(Ly)_$(D)_$(θ/pi)_$(ϕ/pi)_$(K)_Gamma.pdf",fig)
# end
