...
lsχ[iΓ′,:] = [(lsM[i+1] - lsM[i]) for i in 1:length(lsM)-1]

end

lscHf = (lsHf[1:end-1] + lsHf[2:end])/2
...
display(fig)
# # lsχ
save("$(figurename)/stack_chi_M_$(Lx)x$(Ly)_$(D)_$(θ/pi)_$(ϕ/pi).png",fig)
save("$(figurename)/stack_chi_M_$(Lx)x$(Ly)_$(D)_$(θ/pi)_$(ϕ/pi).pdf",fig)
