...
for (iθ,θ) in enumerate([0.0,0.5] * pi), (i,Hf) in enumerate(lsHf)
...
display(fig)
# # lsχ
save("$(figurename)/compare_M_ZZHC_Gp_$(Lx)x$(Ly)_$(D).png",fig)
save("$(figurename)/compare_M_ZZHC_Gp_$(Lx)x$(Ly)_$(D).pdf",fig)
