...
figurename = "NCTO/KitaevGamma-cubic/figures/ZZHC/RC3"
tailname = ""
...
lsk = map(x -> coordinate(x/2;basis = KBASIS2),[[1,0],[0,1],-[1,0],-[0,1]])
...
xlabel = L"h",ylabel = L"\tilde{S}(q)",
title = "$(Ly)x$(Lx), D=$(D), M_UD")
...
save("$(figurename)/M_UD_$(Lx)x$(Ly)_$(D)_$(params1_Kitaev).png",fig)
save("$(figurename)/M_UD_$(Lx)x$(Ly)_$(D)_$(params1_Kitaev).pdf",fig)
