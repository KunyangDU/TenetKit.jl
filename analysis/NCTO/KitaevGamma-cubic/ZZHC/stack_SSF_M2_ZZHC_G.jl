...
lsHf = 0:0.01:0.4
lsk = map(x -> coordinate(x/2;basis = KBASIS2),[[1,2],[2,1],[1,-1],[-1,1],-[1,2],-[2,1]])
...
lsSSFp[iΓ,i] = FSS |> x -> sum(x)/length(x)
end
end
...
save("$(figurename)/stack_SSF_$(Lx)x$(Ly)_$(D)_$(θ/pi)_$(ϕ/pi)_$(K)_M2.png",fig)
save("$(figurename)/stack_SSF_$(Lx)x$(Ly)_$(D)_$(θ/pi)_$(ϕ/pi)_$(K)_M2.pdf",fig)
