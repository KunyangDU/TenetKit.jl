...
# lsHf = 0:0.004:0.1 # θ = 0.5*pi
lsHf = 0:0.02:0.8 # θ = 0.0*pi
...
lsΓ = 0:0.05:0.3
lsSE = zeros(length(lsΓ),length(lsHf))
...
scatterlines!(ax,lsHf,lsSE[i,:] .+ i*2.5,label = "Γ=$(Γ)")
...
save("$(figurename)/stack_SE_$(Lx)x$(Ly)_$(D)_$(θ/pi)_$(ϕ/pi)_$(K).png",fig)
save("$(figurename)/stack_SE_$(Lx)x$(Ly)_$(D)_$(θ/pi)_$(ϕ/pi)_$(K).pdf",fig)
# end
