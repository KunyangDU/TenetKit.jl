...
lsTEE = zeros(length(lsHf),length(lsΓ))
...
hm = heatmap!(ax,lsHf,-lsΓ,-lsTEE / log(2);colormap = :Blues,colorrange = (0.,0.4))

Colorbar(fig[1,2],hm,label = L"-\gamma_E\ /\ \ln 2")

resize_to_layout!(fig)
display(fig)

save("$(figurename)/TEE_h_Γ_$(L)_$(D)_$(params1_Kitaev)_$(θ/pi)_$(ϕ/pi).png",fig)
save("$(figurename)/TEE_h_Γ_$(L)_$(D)_$(params1_Kitaev)_$(θ/pi)_$(ϕ/pi).pdf",fig)
