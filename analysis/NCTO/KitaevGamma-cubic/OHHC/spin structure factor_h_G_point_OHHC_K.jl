...
hmt = heatmap!(axt,lsHf,-lsΓ,lsFFTS;colormap = :Reds,colorrange = (4,5))
# boundary!(axt,FBZpoint;color = :white,linewidth = 1.5,linestyle = :dash)
# boundary!(axt,MFBZpoint;linewidth = 3,breathing=1.015)

Colorbar(fig[1,2],hmt;label = L"\tilde{S}(M/3)")


resize_to_layout!(fig)
display(fig)

save("$(figurename)/structure factor_h_G_point_K_$(L)_$(D)_$(params_Kitaev).png",fig)
save("$(figurename)/structure factor_h_G_point_K_$(L)_$(D)_$(params_Kitaev).pdf",fig)
