...
for kdc in flux_key, k in keys(obsdata[kdc])
    ...
    # site = sum(collect.(map(x -> coordinate(Latt,x),k[1]))) / 6
    # @show findmin(x -> norm(x - site), flux_center_sites)
    # @show map(x -> norm(x - site), flux_center_sites)
    # findmin(x -> norm(x - site), flux_center_sites)[1] < 1e-8 && push!(fluxvalues,dc[k])
    push!(fluxvalues,obsdata[kdc][k])
end
# @show (keys(obsdata))
lsflux[i] = fluxvalues*2^6 |> x -> sum(x)/length(x)
# @show fluxvalues
end
...
xlabel = "h",ylabel = L"\langle W_p\rangle")
