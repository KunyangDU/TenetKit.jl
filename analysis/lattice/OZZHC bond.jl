...
Lx = 4
Ly = 3
Latt = OpenZZHoneyComb(Lx,Ly)  

ut = vcat(2:2:2Ly,reverse(1:2:2Ly+1),reverse(2Ly+2:2:4Ly+2),2Ly+3:2:4Ly+1)
pinds = invperm(vcat([(i-1)*(4Ly+2) .+ ut for i in 1:Lx]...))
permute!(Latt,pinds)
...
bonds = getxyzbonds(Latt, direction=[[sqrt(3)/2,1/2],[sqrt(3)/2,-1/2],[0,1]])
