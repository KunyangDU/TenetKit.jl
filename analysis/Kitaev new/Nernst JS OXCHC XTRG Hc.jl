...
for i in 1:N
    ...
    for k in 1:3
        ...
        for (j,l) in bonds[k]
            if j in yupedgepoints || l in yupedgepoints
                ans = 0.0
                for (jeff,(α,β)) in cinds
                    ans += (real(obs[(Snames[α],Snames[β])][(j,l)]) * jeff)
                end
                lsJedge[iν,i] += ans / (2length(yedgepoints)) * dot(relaVec(Latt,j,l),proj)
            end
            if j in ydownedgepoints || l in ydownedgepoints
                ans = 0.0
                for (jeff,(α,β)) in cinds
                    ans += (real(obs[(Snames[α],Snames[β])][(j,l)]) * jeff)
                end
...
scatterlines!(ax,1 ./ lsβeff,lsJedge ./ Hc,label = "Hc = $(Hc)")
end
...
# f = curve_fit(FermionJS,1 ./ lsβeff[end-tail-fitlength+1:end-tail],lsJedge[end-tail-fitlength+1:end-tail],randn(3))
# # # [-0.001,10,-100,0.01]
# cx = 10 .^ range(log10.([1/4096,0.01])...,100)
# lines!(ax,cx,FermionJS(cx,f.param),color = :red,linewidth = 3)
...
display(fig)
save("Kitaev new/figures/Nernst_single_JS_XTRG_HC_$(Lx)x$(Ly)_$(D)_$(params)_$(proj).pdf",fig)
save("Kitaev new/figures/Nernst_single_JS_XTRG_HC_$(Lx)x$(Ly)_$(D)_$(params)_$(proj).png",fig)
