...
Snames = ("Sx","Sy","Sz")
@load "$(dataname)/data_$(Lx)x$(Ly)_$(D)_$(params)_$(ν)_$(ind).jld2" data
...
figsize = (width = 80*Lx,height = 80*Ly*sqrt(3))
...
for k in 1:3,(j,l) in bonds[k]
    ...
    c = log(abs(JS[j,l]))
    arrow0!(ax,x,y,u,v;arrowsize = 0.5,linewidth = 2,color = get(colorschemes[:Greens],c,(-8,-6)))
end

Colorbar(fig[1,2],colormap = colorschemes[:Greens],colorrange= (-8,-6),
label = L"\ln\ J_S")
