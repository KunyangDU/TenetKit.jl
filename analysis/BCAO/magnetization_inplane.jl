...
scatterlines!(ax,lsHx * 6.54,lsSx,label = L"\mathbf{\hat{x}}")

scatterlines!(ax,lsHy * 6.54,lsSy,label = L"\mathbf{\hat{y}}")
...
scatterlines!(inset_ax,lsHy * 6.54,lsSy)
...
save("BCAO/figures/Magnetization inplane_$(Lx)x$(Ly)_$(D)_$(length(lsHx)).png",fig)
save("BCAO/figures/Magnetization inplane_$(Lx)x$(Ly)_$(D)_$(length(lsHx)).pdf",fig)
