...
intensity = 0.3 / maximum(sum((@. sqrt(Sz^2 + Sy^2)))/length(Sy))
...
display(fig)
save("$(figurename)/spin pattern_$(Lx)x$(Ly)_$(D)_$(params).png",fig)
save("$(figurename)/spin pattern_$(Lx)x$(Ly)_$(D)_$(params).pdf",fig)