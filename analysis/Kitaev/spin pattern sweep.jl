...
title = "$(2Lx)x$(2Ly) Kitaev model, D=$(D), $(params)")

plotLatt!(ax,Latt,[0,1];site = true,sitelabel = false)
plotbond!(ax,Latt,xbonds,-4ssx,[0,1];colorlimit = (0.,1.),linewidth = 8,colormap = :Blues)
plotbond!(ax,Latt,ybonds,-4ssy,[0,1];colorlimit = (0.,1.),linewidth = 8,colormap = :Greens)
plotbond!(ax,Latt,zbonds,-4ssz,[0,1];colorlimit = (0.,1.),linewidth = 8,colormap = :Reds)

Colorbar(fig[2,1],limits = (0,1),width = Lx*sqrt(3)*18,height = 10,vertical=false,flipaxis = false,colormap = :Blues,label = L"\langle S_xS_x\rangle")
Colorbar(fig[2,2],limits = (0,1),width = Lx*sqrt(3)*18,height = 10,vertical=false,flipaxis = false,colormap = :Greens,label = L"\langle S_yS_y\rangle")
Colorbar(fig[2,3],limits = (0,1),width = Lx*sqrt(3)*18,height = 10,vertical=false,flipaxis = false,colormap = :Reds,label = L"\langle S_zS_z\rangle")
