...
for (i,λ) in enumerate(lsλ)
    ...
    selected_bonds = bonds[selected_site .- 1]
    f = curve_fit(model,selected_site,selected_bonds,p0)
    @show norm(f.resid ./ selected_bonds),f.param
    lines!(ax,site,model(site,f.param),color = :green,linestyle = :dash)
...
save("J1J2chain/figures/SSdecay-p1_$(Lx)x$(Ly).pdf",fig)
save("J1J2chain/figures/SSdecay-p1_$(Lx)x$(Ly).png",fig)
