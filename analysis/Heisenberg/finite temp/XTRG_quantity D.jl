...
for i in eachindex(lsβ)
    ...
    # lsE[iν,i] = min(data["E"] / size(Latt), minimum(lsE[iν,1:i]))
...
lsS = (lsE .- lsF) .* lsβtotal / log(2)
scatterlines!(axE, 1 ./ lsβtotal, lsE,label = "D = $(D)")
scatterlines!(axS, 1 ./ lsβtotal, lsS)
scatterlines!(axC, 1 ./ lsβtotal[2:end], lsC)

end
for ax in [axS,axE,axC]
xlims!(ax,10. ^ (-2),10. ^ (-0.5))
end
# ylims!(axF,1e-11,1e-7)
ylims!(axS,-0.1,0.25)
ylims!(axE,-0.645,-0.63)
ylims!(axC,-0.1,0.2)
...
display(fig)
save("Heisenberg/figures/XTRG_BENCHMARK_D_$(Lx)x$(Ly)_$(params).pdf",fig)
save("Heisenberg/figures/XTRG_BENCHMARK_D_$(Lx)x$(Ly)_$(params).png",fig)
