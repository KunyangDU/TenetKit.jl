...
As[ik,:] = cfe(data["a"],data["b"][1:end-1],data["d"],lsω;η = 0.1,K = 40)
end
...
display(fig)
save("$(figurename)/Akomega_$(Lx)x$(Ly)_$(D)_$(params)_$(k)_$(N).png",fig)
save("$(figurename)/Akomega_$(Lx)x$(Ly)_$(D)_$(params)_$(k)_$(N).pdf",fig)
