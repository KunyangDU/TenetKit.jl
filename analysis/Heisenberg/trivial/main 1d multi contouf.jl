...
# hm = heatmap!(ax,lskx,lsω,sum([dySS[:,:,i] for i in 1:3]);colorrange = (0,3),colormap = :Reds)
contourf!(ax,lskx,lsω,sum([dySS[:,:,i] for i in 1:3]);
levels = range(0, 3, length = 40),
extendhigh = get(colorschemes[:Reds],1) ,
colormap = :Reds)
...
save("Heisenberg/trivial/figures/dynamics_ctf_$(Lx)x$(Ly)_$(D)_$(params).pdf",fig)
save("Heisenberg/trivial/figures/dynamics_ctf_$(Lx)x$(Ly)_$(D)_$(params).png",fig)
