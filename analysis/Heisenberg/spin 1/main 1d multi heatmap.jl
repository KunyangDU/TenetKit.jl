...
lskxc = pi * range(0,2,100)
ω(x) = @. sqrt(Δ^2 + v^2 * sin(x)^2)
lsω = range(0,2 * pi,100)
...
for (ik,k) in enumerate(lsk),i in 1:length(tnode)-1
    for (in,n) in enumerate(("x","y","z"))
        @load "$(dataname)/lsSS_x_$(Lx)x$(Ly)_$(D)_$(params)_$(round.(k / pi,digits = 8))_$(tnode[i])_$(tnode[i+1]).jld2" lsSS
...
hm = heatmap!(ax,norm.(lsk),lsω,sum([dySS[:,:,i] for i in 1:3]))
...
lines!(ax,lskxc,v*abs.(sin.(lskxc)),color = :grey, linestyle = :dash)
lines!(ax,lskxc,ω(lskxc),color = :white,linewidth = 1)
